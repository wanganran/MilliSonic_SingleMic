...
/**
  * Created by anwang on 7/7/2017.
  */

class FIRFilter(val sampleRate: Int) {

  trait Filter {
    def apply(sig: Array[Float], lastSig: Array[Float], res: Array[Float]): Unit
  }
  ...
  def getStreamFIRFilter(halfWind:Int, f1:Int, f2:Int, initSig:Array[Float]):Float=>Float= {
    val mask = new Array[Float](halfWind * 2 + 1)
    getHammingWindow(mask)
    for (i <- 0 until halfWind * 2 + 1) {
      val n = halfWind - i
    ...
    val slidingWindow = new Array[Float](mask.length)
    for (i <- Math.max(0, initSig.length - slidingWindow.length) until initSig.length)
      slidingWindow(slidingWindow.length - initSig.length + i) = initSig(i)
    var off = 0

    { s: Float =>
      slidingWindow(off%slidingWindow.length) = s
      var sum = 0f
      for (i <- mask.indices) {
        sum += slidingWindow((i + off + 1) % slidingWindow.length) * mask(i)
      }
      off += 1
      sum
    }
